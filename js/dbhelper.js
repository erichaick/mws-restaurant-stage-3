  ...
  static openDatabase() {
    return idb.open("MWS", 3, function (upgradeDb) {
      switch (upgradeDb.oldVersion) {
        case 0:
        case 1:
          var restaurants = upgradeDb.createObjectStore('restaurants', {keyPath: 'id'});
 ...
 */
  static updateReviews(restaurantId) {
    return new Promise((resolve, reject) => {
      ...
      fetch(DBHelper.DATABASE_URL + '/reviews?restaurant_id=' + restaurantId).then(response => {
        response
          .json()
          .then(data => {
            DBHelper
            ...
            var event = new CustomEvent("reviews_updated", {
              detail: {
              ...
              .then(db => {
                ...
                restaurants.forEach(element => {
                  element.is_favorite = element.is_favorite
                    ? (element.is_favorite.toString() == "true"
                      ? true
                      : false)
                    : false;
                  store.put(element);
